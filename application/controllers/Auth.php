...
class Auth extends CI_Controller
{
	...
	public function delete_user($id = '')
	{
		...
		{
			$this->data['error'] = true;
			$this->data['message'] = 'You must be an administrator to take this action.';
			echo json_encode($this->data);
			return false;
		}
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() === TRUE && $new_user_id = $this->ion_auth->register($identity, $password, $email, $additional_data, $group))
		{
			...
			$this->email->subject('Welcome to the '.company_name());  
			$body = "<html>
				<body>
					<p>Welcome to the ".company_name().", This is an automatically generated email to inform you. Below are the credentials for your work dashboard..</p>
					<p>Login credentials</p>
					<p>Email: ".$this->input->post('email')."</p>
					<p>Password: ".$this->input->post('password')."</p>
					<p><a href='".base_url()."'>Login Now</a></p>
				</body>
			</html>";
			$this->email->message($body); 
	  		$this->email->send();
		  	if($this->input->post('new_register')){
				$msg = "User registered successfully. Go to the login page and login with your credentials.";
			}else{
				...
				if ($this->ion_auth->update($user->id, $data))
				{
					...
					if ($this->input->post('plan_id'))
					{
						$users_plans_data = array(
							...
							'end_date' => format_date($this->input->post('end_date'),"Y-m-d"),		
							'expired' => date("Y-m-d") > format_date($this->input->post('end_date'),"Y-m-d")?0:1,		
						);
